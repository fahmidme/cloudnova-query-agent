...
`python -m app evaluate --live [--provider openai|anthropic]` makes seven sequential provider calls over the original fixture: the six stakeholder query cases above plus period churn. Assert the independently specified answer rows, requested column contract and deterministic ordering. Under AGENT.md, missing-evidence replies are conversational: the unsupported case must avoid SQL and mention missing event/date evidence and opening/cohort evidence. This is a limited text check, not a semantic guarantee; inspect the live explanation manually. Record SQL, result, timing, and pass/fail per case. A provider error must remain a failure; stop subsequent calls after a provider failure to avoid repeated billing/authentication failures. This is a small regression set, not an accuracy benchmark. Without `--live`, evaluate curated SQL offline and mark natural-language unsupported recognition as untested.
